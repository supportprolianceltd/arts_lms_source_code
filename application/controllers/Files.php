...
class Files extends CI_Controller
{
    ...
    function index()
    {
        ...
        if (isset($_GET['course_id']) && isset($_GET['lesson_id']) && $user_id > 0) {
            ...
            if (enroll_status($course_id) == 'valid' || $this->session->userdata('admin_login') == '1' || in_array($user_id, $multi_instructors) || ($lesson_id==-1 && $course_details['course_overview_provider']=='system')) {


                //Assign video url
                if($lesson_id==-1) $fileUrl = $course_details['video_url'];
                ...
                if ($get_lesson_type == 'image_file' || $get_lesson_type == 'pdf_file' || $get_lesson_type == 'pdfs_file' || $get_lesson_type == 'text_file') {
                    ...
                    readfile($fileUrl);
                    exit;
                } elseif ($get_lesson_type == 'video_file' || $get_lesson_type == 'amazon_video_url' || $get_lesson_type == 'academy_cloud' || $get_lesson_type == 'html5_video_url' || $get_lesson_type == 'audio_file' || $lesson_id==-1) {
