...
class Home extends CI_Controller
{
    ...
    public function course($slug = "", $course_id = "")
    {
        ...
        if($course_details){
            ...
            $page_data['meta_keywords'] = $course_details['meta_keywords'];
            $page_data['meta_description'] = $course_details['meta_description'];
        }
    ...
    public function my_courses($course_id = null)
    {
        ...
        if($course_id){
            ...
            if($enroll_status == 'valid') $page_data['course_id'] = $course_id;
            else {
                $this->session->set_flashdata('error_message', site_phrase('You are not enrolled in this course or your enrollment has expired. You need to buy it again'));
    ...
    public function my_messages_chat($param1 = "", $param2 = "")
    {
        if ($this->session->userdata('user_login') != true) {
            redirect(site_url('home'), 'refresh');
        }
        if ($param1 == 'read_message') {
            ...
            $message_thread_code = $this->crud_model->send_new_private_message();
            $this->session->set_flashdata('flash_message', site_phrase('message_sent'));
            redirect(site_url('home/my_messages_chat/read_message/' . $message_thread_code), 'refresh');
        } elseif ($param1 == 'send_reply') {
            $this->crud_model->send_reply_message($param2); //$param2 = message_thread_code
            $this->session->set_flashdata('flash_message', site_phrase('message_sent'));
            redirect(site_url('home/my_messages_chat/read_message/' . $param2), 'refresh');
        }
        $page_data['page_name'] = "my_messages_chat";
        $page_data['page_title'] = site_phrase('chat');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function update_profile($param1 = "", $is_profile_page = false)
    {
        if ($param1 == 'update_basics') {
            ...
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','update_profile','update_basics','edit_user',null,'user_model','updated_successfully','update profile','successful',['text'=>'attempts to update profile']);
            if (isset($_FILES['user_image']) && $_FILES['user_image']['name'] != "") {
                $param1 = "update_photo";
                $is_profile_page=true;
            }
            else redirect(site_url('home/profile/user_profile'), 'refresh');
        } elseif ($param1 == "update_credentials") {
            $this->user_model->update_account_settings($this->session->userdata('user_id'));
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','update_profile','update_credentials','update_account_settings',null,'user_model','updated_successfully','update account settings','successful',['text'=>'attempts to update account settings']);
            ...
            $this->session->set_flashdata('flash_message', site_phrase('updated_successfully'));
                $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','update_profile','update_photo','upload_user_image',null,'user_model','updated_successfully','upload user image','successful',['text'=>'attempts to upload user image']);
            }
            $this->session->set_flashdata('flash_message', site_phrase('updated_successfully'));
    ...
    public function handleWishList($return_number = "")
    {
        if ($this->session->userdata('user_login') != 1) {
            ...
            if (isset($_POST['course_id'])) {
                ...
                $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','handleWishList',null,'handleWishList',"$course_id",'crud_model',null,'handle Wish List','successful',['text'=>'attempts to handle Wish List']);
    ...
    function toggleWishlistItems($course_id = "", $identifier = "")
    {
        if ($this->session->userdata('user_login') != 1) {
            ...
            if ($response_bool) {
                $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','toggleWishlistItems',null,'handleWishList',"$course_id",'crud_model','Course added to wishlist','added course to wishlist','successful',['text'=>'attempts to added course to wishlist']);
                $response['success'] = get_phrase('Course added to wishlist');
            } else {
                $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','toggleWishlistItems',null,'handleWishList',"$course_id",'crud_model','Course removed from wishlist','removed course from wish list','successful',['text'=>'attempts to remove course from wish list']);
            ...
            $response['text'] = ['elem' => '#wishlistItemsCounter', 'content' => count((array)$my_wishlist_items)];
    ...
    public function handleCartItems($return_number = "")
    {
        ...
        if (in_array($course_id, $previous_cart_items)) {
            ...
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','handleCartItems',null,'unset',"{$previous_cart_items[$key]}",'std','handle Cart Items','handle Cart Items','successful',['text'=>'attempts to handle Cart Items']);
        } else {
            array_push($previous_cart_items, $course_id);
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','handleCartItems',null,'array_push',"$previous_cart_items, $course_id",'std','handle Cart Items','handle Cart Items','successful',['text'=>'attempts to handle Cart Items']);
    ...
    public function handle_cart_items($course_id = "", $identifier = "")
    {
        ...
        if (in_array($course_id, $previous_cart_items)) {
            ...
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','handle_cart_items',null,'unset',"{$previous_cart_items[$key]}",'std','Item successfully removed from cart','removed items from cart','successful',['text'=>'attempts to remove items from cart']);
            ...
            array_push($previous_cart_items, $course_id);
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','handle_cart_items',null,'array_push',"$previous_cart_items, $course_id",'std','Item successfully added to cart','added items to cart','successful',['text'=>'attempts to add items to cart']);
        ...
        $response['text'] = ['elem' => '#cartItemsCounter,.cartItemsCounter', 'content' => count((array)$cart_items)];
    ...
    public function handle_buy_now($course_id = "")
    {
        ...
        if (in_array($course_id, $previous_cart_items)) {
            ...
            array_push($previous_cart_items, $course_id);
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','handle_buy_now',null,'array_push',"$previous_cart_items, $course_id",'std',null,'handle buy now','successful',['text'=>'attempts to handle buy now']);
    ...
    public function handleCartItemForBuyNowButton()
    {
        ...
        if (!in_array($course_id, $previous_cart_items)) {
            array_push($previous_cart_items, $course_id);
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','handleCartItemForBuyNowButton',null,'array_push',"$previous_cart_items, $course_id",'std',null,'handle Cart Item For Buy Now Button','successful',['text'=>'attempts to handle Cart Item For Buy Now Button']);
        }
        $this->session->set_userdata('cart_items', $previous_cart_items);
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/cart_items');
    }
    ...
    public function paypal_payment($user_id = "", $amount_paid = "", $paymentID = "", $paymentToken = "", $payerID = "", $payment_request_mobile = "")
    {
        ...
        if (!$status) {
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','paypal_payment','$status','paypal_payment',"$paymentID, $paymentToken, $payerID, $paypalClientID, $paypalSecret",'payment_model',"an_error_occurred_during_payment",'verify paypal payment','failed',['text'=>'attempts to verify paypal payment']);
        ...
        $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','paypal_payment','$status','paypal_payment',"$paymentID, $paymentToken, $payerID, $paypalClientID, $paypalSecret",'payment_model',"an_error_occurred_during_payment",'verify paypal payment','success',['text'=>'attempts to verify paypal payment']);
...
//uu
    public function users_api(){
        $users=(array)$this->user_model->get_all_user()->result_array();
        $len=count($users);
        for($i=0;$i<$len;$i++) $users[$i]['password']=null;
...
//magic
        ...
        if ($lesson_details && $lesson_details['course_id'] != $course_id && $course_details['course_type'] == 'general') {
    ...
    public function rate_course()
    {
        ...
        if (enroll_status($course_id)) {
            ...
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','rate_course',null,'rate',null,'crud_model',"You have successfully rated the course",'rate course','success',['text'=>'attempts to rate course']);
            ...
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','rate_course',null,'rate',null,'crud_model',"This course cannot be rated. Please buy the course first",'rate course','failed',['text'=>'attempts to rate course']);
    ...
    public function remove_rating($course_id, $rating_id)
    {
        if (enroll_status($course_id) || $this->session->userdata('admin_login')) {
            ...
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','remove_rating',null,'delete',"rating,id,$rating_id",'db',"Review deleted successfully",'delete review','success',['text'=>'attempts to delete review']);
    ...
    public function course_action($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == 'create') {
            if (isset($_POST['create_course'])) {
                ...
                $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','course_actions','create','add_course',null,'crud_model',null,'add course','successful',['text'=>'attempts to add course via course actions']);
                redirect(site_url('home/create_course'), 'refresh');
            } else {
                $this->crud_model->add_course('save_to_draft');
                $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','course_actions','create','add_course','save_to_draft','crud_model',null,'add course','successful',['text'=>'attempts to add course via course actions']);
            ...
            if (isset($_POST['publish'])) {
                ...
                $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','course_actions','edit','update_course',"$param2,publish",'crud_model',null,'edit course','successful',['text'=>'attempts to edit course via course actions']);

                redirect(site_url('home/dashboard'), 'refresh');
            } else {
                $this->crud_model->update_course($param2, 'save_to_draft');
                $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','course_actions','edit','update_course',"$param2,save_to_draft",'crud_model',null,'edit course','successful',['text'=>'attempts to edit course via course actions']);
                redirect(site_url('home/dashboard'), 'refresh');
            }
        }
    }
    ...
    public function sections($action = "", $course_id = "", $section_id = "")
    {
        ...
        if ($action == "add") {
            ...
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','sections','add','add_section',"$course_id",'crud_model',"section_has_been_added_successfully",'add section','successful',['text'=>'attempts to add section']);
        } elseif ($action == "edit") {
            $this->crud_model->edit_section($section_id);
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','sections','edit','edit_section',"$section_id",'crud_model',"section_has_been_updated_successfully",'edit section','successful',['text'=>'attempts to edit section']); 
        } elseif ($action == "delete") {
            $this->crud_model->delete_section($course_id, $section_id);
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','sections','delete','delete_section',"$course_id, $section_id",'crud_model',"section_has_been_deleted_successfully",'delete section','successful',['text'=>'attempts to delete section']);
    ...
    public function manage_lessons($action = "", $course_id = "", $lesson_id = "")
    {
        ...
        if ($action == 'add') {
            ...
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','manage_lessons','add','add_lesson',null,'crud_model','lesson_added','add lesson','successful',['text'=>'attempts to add lesson']);
            $this->session->set_flashdata('flash_message', site_phrase('lesson_added'));
        } elseif ($action == 'edit') {
            $this->crud_model->edit_lesson($lesson_id);
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','manage_lessons','edit','edit_lesson',"$lesson_id",'crud_model','lesson_updated','update lesson','successful',['text'=>'attempts to update lesson']);
            $this->session->set_flashdata('flash_message', site_phrase('lesson_updated'));
        } elseif ($action == 'delete') {
            $this->crud_model->delete_lesson($lesson_id);
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','manage_lessons','delete','delete_lesson',"$lesson_id",'crud_model','lesson_deleted','delete lesson','successful',['text'=>'attempts to delete lesson']);
    ...
    public function get_enrolled_to_free_course($course_id)
    {
        ...
        if ($this->session->userdata('user_login') == 1) {
            ...
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','get_enrolled_to_free_course',null,'enrol_to_free_course',"$course_id",'crud_model',null,'enrol to free course','successful',['text'=>'attempts to enrol to free course']);
    ...
    public function submit_quiz($from = "")
    {
        ...
        $page_data['total_questions'] = count((array)$quiz_questions);
        $page_data['course_id']   = $course_id;
    ...
    function save_quiz_result($course_id = "", $quiz_id = "", $obtained_marks = '')
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','save_quiz_result',null,'update',"watch_histories",'db',null,'save quiz result','successful',['text'=>'attempts to save quiz result']);
        } else {
            ...
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','save_quiz_result',null,'insert',"watch_histories",'db',null,'save quiz result','successful',['text'=>'attempts to save quiz result']);
        }
    }
    ...
    public function get_enrolled_to_free_course_mobile($course_id = "", $user_id = "", $get_request = "")
    {
        if ($get_request == "true") {
            ...
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','get_enrolled_to_free_course_mobile',null,'enrol_to_free_course_mobile',"$course_id, $user_id",'crud_model',null,'enrol to free course mobile','successful',['text'=>'attempts to enrol to free course mobile']);
    ...
    {
        if (count((array)$this->session->userdata('cart_items')) == 0) {
    ...
    function subscribe_to_our_newsletter()
    {
        ...
        if (filter_var($data['email'], FILTER_VALIDATE_EMAIL)) {
            if ($this->db->get_where('newsletter_subscriber', $data)->num_rows() > 0) {
                ...
                $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','subscribe_to_our_newsletter',null,'update',"newsletter_subscriber",'db',"Thanks for subscribing to our newsletter",'subscribe to news letter','successful',['text'=>'attempts to subscribe to news letter']);
            } else {
                $data['created_at'] = time();
                $this->db->insert('newsletter_subscriber', $data);
                $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','subscribe_to_our_newsletter',null,'insert',"newsletter_subscriber",'db',"Thanks for subscribing to our newsletter",'subscribe to news letter','successful',['text'=>'attempts to subscribe to news letter']);

            }
            $response['success'] = get_phrase('Thanks for subscribing to our newsletter');
        } else {
            $response['error'] = get_phrase('Invalid email address');
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','subscribe_to_our_newsletter',null,'insert',"newsletter_subscriber",'db',"Invalid email address",'subscribe to news letter','failed',['text'=>'attempts to subscribe to news letter']);
        }
        echo json_encode($response);
    }
    ...
    function contact_us($param1 = "")
    {
        ...
        if ($param1 == 'submit') {
            ...
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','contact_us',null,'insert',"contact",'db',"Your contact request has been sent successfully",'send contact request','successful',['text'=>'attempts to send contact request']);
    ...
    function dark_and_light_mode()
    {
        if ($this->session->userdata('theme_mode') != 'dark-theme') {
            ...
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','dark_and_light_mode',null,'set_userdata',"theme_mode,dark-theme",'session',null,'set dark theme','successful',['text'=>'attempts to set dark theme']);

        } else {
            $this->session->unset_userdata('theme_mode');
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','dark_and_light_mode',null,'unset_userdata',"theme_mode",'session',null,'unset dark theme','successful',['text'=>'attempts to unset dark theme']);
            echo json_encode(['toggleClass' => ['elem' => 'body', 'content' => 'dark-theme']]);
        }
    }
    ...
    function coupon_offer_100_percent()
    {
        ...
        if (count((array)$cart_items) > 0 && $coupon_code && $coupon_details['discount_percentage'] == 100 && $coupon_details['expiry_date'] >= time() && $enrol_user_id > 0 && $this->session->userdata('user_login')) {
    ...
    function get_my_notification($type = "")
    {
        ...
        if ($type == 'mark_all_as_read') {
            ...
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','get_my_notification','mark_all_as_read','update',"notifications,to_user,$user_id,['status' => 1]",'db',null,'mark all notification as read','successful',['text'=>'attempts to mark all notification as read']);
        ...
        if ($type == 'remove_all') {
            ...
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','get_my_notification','remove_all','delete',"notifications,to_user,$user_id",'db',null,'remove all notifications','successful',['text'=>'attempts to remove all notification']);
        ...
        if ($this->session->userdata('user_login') != 1) {
            ...
            if (isset($_POST) && count($_POST) > 0) :
                ...
                if (in_array(strtolower($ext), $accepted_ext)) {
                    ...
                    $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','become_an_instructor',null,'post_instructor_application',null,'user_model',"Invalide document file",'post instructor application','failed',['text'=>'attempts to post instructor application']);

                    redirect('home/become_an_instructor', 'refresh');
                }

                $this->user_model->post_instructor_application($this->session->userdata('user_id'));
                $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','become_an_instructor',null,'post_instructor_application',null,'user_model',"The request was successful",'post instructor application','successful',['text'=>'attempts to post instructor application']);
    ...
    public function payout_settings($param1 = "")
    {
        ...
        if (isset($_POST['gateways'])) {
            ...
            $this->db->where('id', $this->session->userdata('user_id'));
            $this->db->update('users', $data);
            $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','payout_settings','gateways','update',"users,id",'db',"payment_settings_has_been_updated",'update payment settings','successful',['text'=>'attempts to update paymenet settings']);
    ...
    function account_disable()
    {
        ...
        if (isset($_POST) && count($_POST)) {
            ...
            if ($user_details['password'] == sha1($current_password)) {
                ...
                $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','account_disable',null,'update',"users,id",'db',"Your account has been disabled",'disable account','successful',['text'=>'attempts to disable account']);
            } else {
                $this->crud_model->add_activity_log($this->session->userdata('user_id'),1,'home','account_disable',null,'update',"users,id",'db',"Your account has been disabled",'disable account','failed',['text'=>'attempts to disable account']);
    ...
    public function purchase_now($course_id = "")
    {
        $this->session->set_userdata('cart_items', array());
        ...
        if ($this->session->userdata('user_login')) {
            ...
            set_url_history(site_url('payment'));
            redirect(site_url('login'), 'refresh');
            //set_url_history(site_url('home/shopping_cart'));
            //echo json_encode(['redirectTo' => site_url('login')]);
        }
    ...
    public function all_courses()
    {
        ...
        $page_data['page_title'] = site_phrase("all_courses");
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function completed_courses()
    {
        ...
        $page_data['page_name'] = "completed_courses";
        $page_data['page_title'] = site_phrase("completed_courses");
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function statistics()
    {
        ...
        $page_data['page_title'] = site_phrase("statistics");
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function all_schedules()
    {
        ...
        $page_data['page_name'] = "all_schedules";
        $page_data['page_title'] = site_phrase("all_schedules");
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }

    public function ongoing_schedules()
    {
        if ($this->session->userdata('user_login') != true) {
            redirect(site_url('home'), 'refresh');
        }

        $page_data['page_name'] = "ongoing_schedules";
        $page_data['page_title'] = site_phrase("ongoing_schedules");
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function upcoming_schedules()
    {
        ...
        $page_data['page_name'] = "upcoming_schedules";
        $page_data['page_title'] = site_phrase("upcoming_schedules");
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }

    public function elapsed_schedules()
    {
        if ($this->session->userdata('user_login') != true) {
            redirect(site_url('home'), 'refresh');
        }

        $page_data['page_name'] = "elapsed_schedules";
        $page_data['page_title'] = site_phrase("elapsed_schedules");
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function all_assignments()
    {
        ...
        $page_data['page_name'] = "all_assignments";
        $page_data['page_title'] = site_phrase("all_assignments");
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function pending_assignments()
    {
        ...
        $page_data['page_name'] = "pending_assignments";
        $page_data['page_title'] = site_phrase("pending_assignments");
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function active_assignments()
    {
        ...
        $page_data['page_name'] = "active_assignments";
        $page_data['page_title'] = site_phrase("active_assignments");
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }

    public function elapsed_assignments()
    {
        if ($this->session->userdata('user_login') != true) {
            redirect(site_url('home'), 'refresh');
        }

        $page_data['page_name'] = "elapsed_assignments";
        $page_data['page_title'] = site_phrase("elapsed_assignments");
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }

    public function schedule_info($schedule_id)
    {
        if ($this->session->userdata('user_login') != true) {
            redirect(site_url('home'), 'refresh');
        }

        $page_data['page_name'] = "schedule_info";
        $page_data['page_title'] = site_phrase("schedule_info");
    ...
    public function notifications()
    {
        ...
        $page_data['page_title'] = site_phrase("notifications");
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function category($slug='',$category_id)
    {
        ...
        $page_data['page_title'] = site_phrase("category");
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }

    public function all_category($parent=null,$booking=null)
    {
        $page_data['parent']=$parent;
        $page_data['booking']=$booking;
        $page_data['page_name'] = "all_category";
        $page_data['page_title'] = site_phrase("all_category");
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }

    public function consent_information()
    {
        $page_data['page_name'] = "consent_information";
        $page_data['page_title'] = site_phrase("consent_information");
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }

    public function modern_slavery_statement()
    {
        $page_data['page_name'] = "modern_slavery_statement";
        $page_data['page_title'] = site_phrase("modern_slavery_statement");
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }

    public function book_class()
    {
        $this->all_category(0,1);
        //$page_data['page_name'] = "book_class";
        //$page_data['page_title'] = site_phrase("book_class");
        //$this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }

    public function choose_course()
    {
        $page_data['page_name'] = "choose_course";
        $page_data['page_title'] = site_phrase("choose_course");
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
