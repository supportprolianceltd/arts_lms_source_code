...
$video_controls = lesson_progress($lesson_id)?'controls':null;
?>
...
<?php if ($get_lesson_type == 'youtube_video_url') : ?>
	<div class="<?php if ($full_page) echo 'bg-black'; ?>">
		<div class="plyr__video-embed " id="player">
			<iframe height="500" src="<?php echo $lesson_details['video_url']?str_replace('youtu.be/','www.youtube.com/embed/',$lesson_details['video_url']):null; ?>?origin=https://plyr.io&amp;iv_load_policy=3&amp;modestbranding=1&amp;playsinline=1&amp;showinfo=0&amp;rel=0&amp;enablejsapi=1" allowfullscreen allowtransparency allow="autoplay"></iframe>
		</div>
	</div>
...
<?php elseif ($get_lesson_type == 'google_drive_video_url') : ?>
	...
	<div class=" <?php if ($full_page) echo 'bg-black'; ?>">
		<video id="player" playsinline <?php echo $video_controls;?>>
...
<?php elseif ($get_lesson_type == 'amazon_video_url' || $get_lesson_type == 'wasabi_video_url' || $get_lesson_type == 'academy_cloud' || $get_lesson_type == 'html5_video_url') : ?>
	<div class=" <?php if ($full_page) echo 'bg-black'; ?>">
		<video poster="<?php echo $lesson_thumbnail_url; ?>" id="player" playsinline <?php echo $video_controls;?>>
...
<?php elseif ($get_lesson_type == 'audio_file') : ?>
	...
	<div class="p-3 <?php if ($full_page) echo 'bg-black'; ?>" style="margin-top: 250px;">
		<audio id="player" playsinline <?php echo $video_controls;?>>
...
<?php else:
	if(is_array($completed_lessons) && !in_array($lesson_id, $completed_lessons)) $this->crud_model->update_watch_history_manually($lesson_id, $course_id, $user_id);
?>
	<?php if ($get_lesson_type == 'quiz') : ?>
		<div class="mt-0">
	...
	<?php elseif ($get_lesson_type == 'image_file') : ?>
		<?php $img_size = getimagesize('uploads/lesson_files/' . $lesson_details['attachment']); $img_override=true;?>
		<div class="pol-Yha">
			<div class="pol-Yha-Img">
				<img width="<?php echo $img_size[0] . 'px'; ?>"  src="<?php echo site_url('files?course_id=' . $course_details['id'] . '&lesson_id=' . $lesson_details['id'] . '&type=image'); ?>" />
			</div>
	...
	<?php elseif ($get_lesson_type == 'ppt_file') : ?>
		<!--https://view.officeapps.live.com/op/embed.aspx?src=https://yourdomain.com/path/to/presentation.pptx-->
		<iframe class="embed-responsive-item" width="100%" height="600px" src="https://docs.google.com/gview?url=<?php echo site_url('files?course_id='.$course_id.'&lesson_id='.$lesson_id); ?>&embedded=true" allowfullscreen></iframe>
	<?php elseif ($get_lesson_type == 'doc_file' || $get_lesson_type == 'pdfs_file') : ?>
