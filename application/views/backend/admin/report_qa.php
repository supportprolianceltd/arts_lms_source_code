        ...
        <div class="card">
            ...
            <div class="row justify-content-md-center">
                    <div class="col-12">
                        ...
                        <form class='justify-content-center' method=post  action="<?php echo site_url('admin/report_qa'); ?>">
                    ...
                    <div class='form-group col-md-4'>
                        <input type="text" name="title" class="form-control" value='<?php echo $title;?>' placeholder="<?php echo get_phrase('topic'); ?>">
                    </div>
                ...
                <h4 class="mb-3 mt-4 header-title">Q/A <?php echo get_phrase('reports'); ?></h4>
                <div class="table-responsive mt-4">
                    <table id="basic-datatablei" class="datatable-user-data-buttons table table-striped table-centered mb-0">
                        ...
                        <tbody>
                            <?php
                            foreach ($report_qa as $key => $report) : ?>
                                <tr id='tr_<?php echo $key + 1; ?>'>
                                    <td><?php echo $key + 1; ?></td>
                                    <td><?php $user=$report['user_id']?$this->user_model->get_all_user($report['user_id'])->row_array():[]; echo $user['first_name'].' '.$user['last_name'].'('.$user['email'].')';?></td>
                                    <!--<td><?php $to_user=$report['to_user']?$this->user_model->get_all_user($report['to_user'])->row_array():[]; echo $to_user['first_name'].' '.$to_user['last_name'].'('.$to_user['email'].')';?></td>-->
                                    <td><?php echo $report['title'];?></td>
                                    <td><?php echo date('j F,Y',strtotime($report['created'])); ?></td>
                                    <td><?php echo $report['updated']!='0000-00-00 00:00:00'?date('j F,Y',strtotime($report['updated'])):null; ?></td>
                                    <td>
                                        <div class="dropright dropright">
                                            ...
                                            <ul class="dropdown-menu">
                                                <li><a class="dropdown-item" href="javascript:void(0)" onclick="showAjaxModal('<?php echo site_url('modal/popup/report_qa_info_modal/' . $report['id']);?>','Report');"><?php echo get_phrase('info'); ?></a></li>
                                                <?php if ($report['user_id']==$this->session->userdata('user_id')) : ?>
                                                    <li><a class="dropdown-item" href="javascript:void(0)" onclick="showAjaxModal('<?php echo site_url('modal/popup/report_qa_edit_modal/' . $report['id']);?>','Edit report');"><?php echo get_phrase('edit'); ?></a></li>
                                                    <li><a class="dropdown-item" href="#" onclick="ajax_confirm_modal('<?php echo site_url('admin/report_qa/delete_api/' . $report['id']); ?>','tr_<?php echo $key + 1; ?>');"><?php echo get_phrase('delete'); ?></a></li>
                                                <?php endif;?>
                                            </ul>
                                        </div>
                                    </td>
