  ...
  <div class="col-md-12 mt-2 shadow" style='overflow-y:scroll;max-height:400px;'>
      <?php $user_list = $this->db->where('live_schedule_id', $param2)->get('live_schedule_user')->result_array();
          foreach ($user_list as $live_schedule_user): ?>
            ...
            <?php if($user):?>
                <div id="user_<?php echo $live_schedule_user['id'] ?>" user_id="<?php echo $user['id'] ?>" class='p-1 user_list'><a target='_blank' href='<?php echo site_url("admin/user_profile/{$user['id']}") ?>'><?php echo $user['first_name'].' '.$user['last_name'].'('.$user['email'].')'; ?></a> <button type="button" class="close alignToTitle" aria-label="Close" onclick="ajax_confirm_modal('<?= site_url('admin/live_schedule/delete_user_api/'.$live_schedule_user['id']); ?>', 'user_<?php echo $live_schedule_user['id'] ?>')"><span aria-hidden="true">×</span></button> <hr></div>
            <?php endif;?>
      <?php endforeach; ?>
  </div>
</div>
