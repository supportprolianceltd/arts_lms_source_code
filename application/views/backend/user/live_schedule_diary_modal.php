<?php
  ...
  //$schedules_own=$this->crud_model->fetch_live_schedule_join(null,'start_time',0,$this->session->userdata('user_id'));
  $schedules=$this->crud_model->fetch_live_schedule_join(null,'start_time',1,$this->session->userdata('user_id'),$this->session->userdata('user_id'));
  //$schedules=array_unique(array_merge($schedules,$schedules_own),SORT_REGULAR);
  $schedule_groups=[];
  $time_now=date('d-m-Y');
