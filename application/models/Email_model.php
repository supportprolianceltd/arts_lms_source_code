...
class Email_model extends CI_Model
{
	...
	//
	//atatic email function
	public function send_email($email, $mail_subject = "", $mail_body = "")
	{
		$email_data['subject'] = $mail_subject;
		$email_data['message'] = $mail_body;
		$email_template = $this->load->view('email/static_common_template', $email_data, TRUE);

		return @$this->send_smtp_mail($email_template, $email_data['subject'], $email);
	}

	public function add_user_cv($email, $mail_subject = "", $mail_body = "")
	{
		$email_data['subject'] = $mail_subject;
		$email_data['message'] = $mail_body;
		$email_template = $this->load->view('email/static_common_template', $email_data, TRUE);

		$this->send_smtp_mail($email_template, $email_data['subject'], $email);
	}

	public function course_enrol_notification($student_id = "",$course_ids=array())
	{
	...
	public function create_live_schedule_notification($live_schedule_id){

		//Editable
		$type = 'create_live_schedule';
		//End editable
		$live_schedule=$this->crud_model->get_live_schedule($live_schedule_id);
		$course_id=$live_schedule['course_id'];
		$course_details=$this->crud_model->get_course_by_id($course_id)->row_array();
		//$enrols=$this->crud_model->enrol_history($course_id)->result_array();
		$enrols=$this->crud_model->get_all_live_schedule_user(0,$live_schedule_id)->result_array();
		$instructor_details = $this->user_model->get_all_user($course_details['creator'])->row_array();

		$scheduler_id = $this->session->userdata('user_id');

		$scheduler_details=$this->user_model->get_all_user($scheduler_id)->row_array();
		$prime_admin_details=$this->db->get_where('users', array('role_id' => 1))->row_array();
		$course_creator_details=$this->user_model->get_all_user($course_details['creator'])->row_array();

		$notification = $this->db->where('type', $type)->get('notification_settings')->row_array();

		$replaces['course_title'] = '<a href="'.site_url('home/course/'.slugify($course_details['title']).'/'.$course_details['id']).'" target="_blank">'.$course_details['title'].'</a>';
		$replaces['live_schedule_platform']=$live_schedule['platform'];
		$replaces['live_schedule_start_time']=date('g:ia, j,F,Y',strtotime($live_schedule['start_time']));
		$replaces['live_schedule_duration']=ltrim(substr($live_schedule['duration'],0,5),'0').' hrs';
		$replaces['instructor_name'] = $instructor_details['first_name'].' '.$instructor_details['last_name'];
		$replaces['scheduler_name'] = $scheduler_details['first_name'].' '.$scheduler_details['last_name'];

		foreach(json_decode($notification['user_types'], true) as $user_type){
				
			//Editable
			if($user_type == 'admin'){
				$to_user = $prime_admin_details;
			}

			elseif($instructor_details && $user_type == 'instructor'){
				$to_user = $course_creator_details;
				if($to_user['user_id']==$prime_admin_details['user_id']) continue;
			}

			elseif($user_type == 'scheduler'){
				$to_user = $scheduler_details;
				if($to_user['user_id']==$prime_admin_details['user_id'] || $to_user['user_id']==$course_creator_details['user_id']) continue;
			}
			else continue;
		...
		foreach($enrols as $enrol){
			foreach(json_decode($notification['user_types'], true) as $user_type){
				if($user_type == 'student'){
					$to_user = $this->user_model->get_all_user($enrol['user_id'])->row_array();
				}
				else continue;
				//Editable
	...
	public function update_live_schedule_notification($live_schedule_id){
	...
	public function cancel_live_schedule_notification($live_schedule_id){

		//Editable
		$type = 'cancel_live_schedule';
		//End editable
		$live_schedule=$this->crud_model->get_live_schedule($live_schedule_id);
		$course_id=$live_schedule['course_id'];
		$course_details=$this->crud_model->get_course_by_id($course_id)->row_array();
	...
	public function delete_live_schedule_notification($live_schedule_id,$course_id,$live_schedule_start_time,$live_schedule_duration,$live_schedule_platform){

		//Editable
		$type = 'delete_live_schedule';
		//End editable
		$course_details=$this->crud_model->get_course_by_id($course_id)->row_array();
		//$enrols=$this->crud_model->enrol_history($course_id)->result_array();
		$enrols=$this->crud_model->get_all_live_schedule_user(0,$live_schedule_id)->result_array();
		...
		$replaces['course_title'] = '<a href="'.site_url('home/course/'.slugify($course_details['title']).'/'.$course_details['id']).'" target="_blank">'.$course_details['title'].'</a>';
		$replaces['live_schedule_platform']=$live_schedule_platform;
		$replaces['live_schedule_start_time']=date('g:ia, j,F,Y',strtotime($live_schedule_start_time));
		$replaces['live_schedule_duration']=ltrim(substr($live_schedule_duration,0,5),'0').' hrs';
		$replaces['instructor_name'] = $instructor_details['first_name'].' '.$instructor_details['last_name'];
		$replaces['scheduler_name'] = $scheduler_details['first_name'].' '.$scheduler_details['last_name'];
