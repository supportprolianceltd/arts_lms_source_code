...
class Crud_model extends CI_Model
{
    ...
    public function add_category()
    {
        ...
        $data['booking'] = (int)$this->input->post('booking');
   ...
   // Category Sub Category Image Delete
    ...
    public function add_course($param1 = "")
    {
        ...
        if ($this->input->post('course_overview_provider')=='system' && isset($_FILES['course_overview_url']) && $_FILES['course_overview_url']['name'] != "") {
            if (!file_exists('uploads/lesson_files/videos')) {
                mkdir('uploads/lesson_files/videos', 0777, true);
            }
            $max_size=0;
            $accepted_ext = array('mp4', 'webm');
            $path = $_FILES['course_overview_url']['name'];
            $size = $_FILES['course_overview_url']['size'];
            $dir='uploads/lesson_files/videos/';
            $ext = strtolower(pathinfo($path, PATHINFO_EXTENSION));
            if (in_array($ext, $accepted_ext)) {
                if(($size<=$max_size&&$size!=0)||!$max_size){
                    do{
                        $video_custom_name = random(15) . '.' . $ext;
                    }
                    while(file_exists($dir.$video_custom_name));
                
               
                $data['video_url'] = $dir.$video_custom_name;
                $data['course_overview_provider'] = "system";
                
                move_uploaded_file($_FILES['course_overview_url']['tmp_name'], $dir. $video_custom_name);
                }
                else{
                    $this->session->set_flashdata('error_message', get_phrase('course_overview_video_file_size_exceeded'));
                }
            } else {
    ...
    function trim_and_return_json($untrimmed_array = [])
    {
        ...
        $trimmed_array = array();
        if (sizeof((array)$untrimmed_array) > 0) {
    ...
    public function update_course($course_id, $type = "")
    {
        ...
        } elseif ($this->input->post('course_overview_provider')!='system') {
            $data['course_overview_provider'] = "";
        }
        ...
        if ($this->input->post('course_overview_provider')=='system' && isset($_FILES['course_overview_url']) && $_FILES['course_overview_url']['name'] != "") {
            ...
            if (in_array($ext, $accepted_ext)) {
                if(($size<=$max_size&&$size!=0)||!$max_size){
                    ...
                    if($course_details['course_overview_provider']=='system' && $course_details['video_url']) if(file_exists($course_details['video_url'])) unlink($course_details['video_url']);
    ...
    function get_course_thumbnail_url($course_id, $type = 'course_thumbnail',$resize=true)
    {
        ...
        if (file_exists('uploads/thumbnails/course_thumbnails/optimized/' . $type . '_' . get_frontend_settings('theme') . '_' . $course_id . $last_modified . '.jpg') && $resize) {
            ...
            //resizeImage
            if($resize) resizeImage('uploads/thumbnails/course_thumbnails/' . $type . '_' . get_frontend_settings('theme') . '_' . $course_id . $last_modified . '.jpg', 'uploads/thumbnails/course_thumbnails/optimized/', 400);
    ...
    public function delete_course($course_id = "")
    {
        ...
        if ($course_type == 'general') {
            // LET LESSONS DELETE SELF
            $course_lessons = $this->crud_model->get_lessons('course', $course_details['id'])->result_array();
            foreach ($course_lessons as $course_lesson) $this->delete_lesson($course_lesson['id']);
    ...
    public function add_lesson()
    {
        ...
        if ($lesson_type == 'video') {
            ...
            if ($attachment_type == 'iframe') {
                ...
                $data['attachment'] = extract_iframe_src($this->input->post('iframe_source'));
            } else {
        ...
        if ($lesson_type == 'video') {
            ...
            if (isset($_FILES['video_file_for_wasabi_storage']) && !empty($_FILES['video_file_for_wasabi_storage']['name'])) {
                ...
                try {
                    ...
                    $previous_file_path_arr = explode("/", $previous_data['video_url']);
                    $previous_file_path_last_index = count((array)$previous_file_path_arr) - 1;
                    $previous_file_path = $previous_file_path_arr[$previous_file_path_last_index - 2] . '/' . $previous_file_path_arr[$previous_file_path_last_index - 1] . '/' . $previous_file_path_arr[$previous_file_path_last_index];
                    $this->wasabi_storage_file_delete($previous_file_path);
                } catch (Exception $e) {
                    // Handle errors
    ...
    public function enrol_a_student_manually()
    {
        ...
        foreach ($users_id as $user_id) {
            ...
            $this->email_model->course_enrol_notification($user_id,$courses_id);
        }
    ...
    {
        if (sizeof((array)$this->input->post('options')) != $this->input->post('number_of_options')) {
    ...
    function manage_fill_in_the_blank_question($quiz_id, $question_id, $action)
    {
        ...
        $data['number_of_options']  = count((array)json_decode($data['correct_answers']));
    ...
    function update_watch_history_manually($lesson_id = "", $course_id = "", $user_id = "")
    {
        ...
        if ($query->num_rows() > 0) {
            $lesson_ids = array_values(array_unique((array)json_decode($query->row('completed_lesson'), true)));
            $total_lesson_qry = $this->db->get_where('lesson', array('course_id' => $course_id));
            $total_lesson_result=$total_lesson_qry->result_array();
            $total_lesson_ids=[];
            foreach($total_lesson_result as $lesson) $total_lesson_ids[]=$lesson['id'];
            $total_lesson = $total_lesson_qry->num_rows();
            $lesson_ids=array_intersect($lesson_ids,$total_lesson_ids);
...
//magic
                if (($key = array_search($lesson_id, $lesson_ids)) !== false && false) {
    ...
    function update_watch_history_with_duration()
    {
        ...
        $lesson_total_seconds = htmlspecialchars_($this->input->post('total_duration'));
        ...
        $lesson_total_seconds = $lesson_total_seconds?$lesson_total_seconds:($lesson_total_duration[0] * 3600) + ($lesson_total_duration[1] * 60) + $lesson_total_duration[2];
    ...
    function checkBbbMeetingExists($endpoint, $meeting_id, $query_data = array())
    {
        ...
        $query_data = count((array)$query_data) > 0 ? $query_data : ['meetingID' => $meeting_id];
    ...
    function get_settings(){
        $rows=array();
        $qry=$this->db->get('settings')->result_array();
        if($qry) foreach ($qry as $row) $rows[$row['key']]=$row['value'];
        return $rows;
    }

    function get_frontend_settings(){
        $qry=$this->db->get('frontend_settings')->result_array();
        if($qry) foreach ($qry as $row) $rows[$row['key']]=$row['value'];
        return $rows;
    }
    ...
    public function enrol_a_student_api($user_id,$courses_id)
    {
        if($courses_id && !is_array($courses_id)) $courses_id[0]=$courses_id;
    ...
    function enrol_students_from_csv(){
        $resp=array('success'=>0,'error'=>0,'total'=>0);

        $course_id = $this->input->post('course_id');

        $csv_arr=$this->fromCSV();
        ...
        foreach($csv_arr as $data) {
            ...
            if($email){
                ...
                if($user_id){
                    $response=$this->enrol_a_student_api($user_id,$course_id);
    ...
    function get_top_courses_categories($category_column = "category_id",$is_top_course=false)
    {
        $query = $this->db
            ->select($category_column . ", count(*) AS course_number", false)
            ->from("course")
            ...
            ->order_by("course_number", "DESC")
            ->where('status', 'active');
            if($is_top_course) $query=$query->where('is_top_course', 1);
            $query=$query->get();
        return $query->result_array();
    }

    public function fetch_categories($parent = null,$booking=null)
    {
        if($booking!==null) {
            $this->db->where('booking', (int)$booking);
        }
        if ($parent!==null && $parent==0) $this->db->where('parent', 0);
        elseif($parent!==null && $parent) $this->db->where('parent>', 0);
        return $this->db->get('category')->result_array();
    }

    function active_course_categories($parent = "",$booking=null){
        $categories=array();
        $all_categories=$this->fetch_categories($parent,$booking);
        if($all_categories){
            foreach($all_categories as $category) {
                $no_course=count((array)$this->get_category_courses($category['id']));
                if($no_course) {
                    $category['no_course']=$no_course;
                    $categories[]=$category;
                }
            }
        }
    ...
    function fetch_watch_histories($user_id = 0, $course_id = 0,$progress=0)
    {
        if ($user_id > 0) {
            $this->db->where('student_id', $user_id);
        }
        ...
        if ($progress) {
            $this->db->where('course_progress>', $progress);
        }
        return $this->db->get('watch_histories');
    }

    function toHexId($id){
        $HEX_ID_OFFSET=1900000;
        return $id?strtoupper(str_pad(dechex($HEX_ID_OFFSET+$id),6,'0',STR_PAD_LEFT)):null;
    }

    function toId($id){
        $HEX_ID_OFFSET=1900000;
        return $id?strtoupper(str_pad($id,6,'0',STR_PAD_LEFT)):null;
    }

    function add_live_schedule(){
        $user_id=$this->session->userdata('user_id');
        $course_id=(int)$this->input->post('course_id');
        $start_time=htmlspecialchars_($this->input->post('start_time'));
        $duration=htmlspecialchars_($this->input->post('duration'));
        $platform=htmlspecialchars_($this->input->post('platform'));
        $title=htmlspecialchars_($this->input->post('title'));
        $platform_link=htmlspecialchars_($this->input->post('platform_link'));
        $note=$this->input->post('note');
        if($user_id && $start_time && $title){
            $data=array('user_id'=>$user_id,'course_id'=>$course_id,'start_time'=>$start_time,'duration'=>$duration,'platform'=>$platform,'title'=>$title,'platform_link'=>$platform_link,'note'=>$note);
            $this->db->insert('live_schedule',$data);

            $live_schedule_id = $this->db->insert_id();
            if($live_schedule_id) $this->email_model->create_live_schedule_notification($live_schedule_id);

            $this->session->set_flashdata('flash_message', get_phrase('schedule_has_been_created'));
        }
        else $this->session->set_flashdata('error_message', get_phrase('schedule_could_not_be_created'));
        return $live_schedule_id;
    }

    function update_live_schedule($id,$active='',$filter=false){
        $course_id=(int)$this->input->post('course_id');
        $start_time=htmlspecialchars_($this->input->post('start_time'));
        $duration=htmlspecialchars_($this->input->post('duration'));
        $platform=htmlspecialchars_($this->input->post('platform'));
        $platform_link=htmlspecialchars_($this->input->post('platform_link'));
        $platform_record_link=htmlspecialchars_($this->input->post('platform_record_link'));
        $title=htmlspecialchars_($this->input->post('title'));
        $note=$this->input->post('note');
        if($id){
            $live_schedule=$this->get_live_schedule($id);
            $data=array('start_time'=>$start_time,'duration'=>$duration,'platform'=>$platform,'title'=>$title,'platform_record_link'=>$platform_record_link,'platform_link'=>$platform_link,'note'=>$note);
            if(strlen($active)) $data['active']=$active;
            if($filter) $data=array_filter($data);
            $this->db->where('id', $id);
            $this->db->update('live_schedule',$data);

            $num_rows=$this->db->affected_rows();
            if($num_rows) {
                if(strlen($active) && $live_schedule['active'] && !$active) $this->email_model->cancel_live_schedule_notification($id);
                if($course_id && $course_id!=$live_schedule['course_id']) $this->email_model->create_live_schedule_notification($id);
                $this->email_model->update_live_schedule_notification($id);
            }

            $this->session->set_flashdata('flash_message', get_phrase('schedule_has_been_updated'));
        }
        else $this->session->set_flashdata('error_message', get_phrase('referenced_schedule_was_not_found'));
    }

    function fetch_live_schedule($active=null,$user_id=0,$course_ids=0,$start_time=0,$end_time=0,$end_time_2=0,$duration=0,$platform=null,$order_by=null,$_pol=0){
        $resp=array();
        ...
        foreach($course_ids as $course_id){
            ...
            if($end_time) $data['ADDTIME(start_time,duration)>']=$end_time;
            if($end_time_2) $data['ADDTIME(start_time,duration)<']=$end_time_2;
            $db=($order_by)?$this->db->order_by($order_by,$_pol?'ASC':'DESC'):$this->db;
            $resp=array_merge($resp,$db->get_where('live_schedule',$data)->result_array());
        }
        //var_dump($resp);
        return $resp;
    }

    function fetch_live_schedule_join($active=null,$order_by='id',$_pol=0,$user_id=0,$participant_id=0,$course_id=0,$start_time=0,$end_time=0,$end_time_2=0,$duration=0,$platform=null){
        if(strlen($active)) $data['live_schedule.active']=$active;
        if($course_id===true) $data['live_schedule.course_id>']=1;
        elseif(strlen($course_id)) $data['live_schedule.course_id']=$course_id;
        if($start_time) $data['live_schedule.start_time>']=$start_time;
        if($duration) $data['live_schedule.duration']=$duration;
        if($platform) $data['live_schedule.platform']=$platform;
        if($end_time) $data['ADDTIME(live_schedule.start_time,live_schedule.duration)>']=$end_time;
        if($end_time_2) $data['ADDTIME(live_schedule.start_time,live_schedule.duration)<']=$end_time_2;

        //$join_data='live_schedule_user.live_schedule_id=live_schedule.id';
        //if($participant_id) $join_data." AND live_schedule_user.user_id=$participant_id";

        //var_dump($participant_id);

        $this->db->select('live_schedule.*')
        ->from('live_schedule');
        if($participant_id) {
            $join_cond="live_schedule_user.live_schedule_id=live_schedule.id AND live_schedule_user.user_id=$participant_id";
            if($user_id) $join_cond." AND live_schedule.user_id=$user_id";
            $this->db->join('live_schedule_user', $join_cond);
        }
        else if($user_id) $data['live_schedule.user_id']=$user_id;
        if($data) $this->db->where($data);
        if($order_by) $this->db->order_by('live_schedule.'.$order_by,$_pol?'ASC':'DESC');
        //echo ' '.$this->db->get_compiled_select();
        //echo ' '.$this->db->last_query();
        return $this->db->get()->result_array();
    }

    function get_live_schedule($id){
        if($id) return $this->db->get_where('live_schedule',array('id'=>$id))->row_array();
    }

    function delete_live_schedule($id){
        if($id) {
            $live_schedule=$this->get_live_schedule($id);
            ...
            if($this->db->affected_rows()) $this->email_model->delete_live_schedule_notification($id,$live_schedule_course_id,$live_schedule_start_time,$live_schedule_duration,$live_schedule_platform);

            $this->session->set_flashdata('flash_message', get_phrase('schedule_has_been_deleted'));
        }
        else $this->session->set_flashdata('error_message', get_phrase('referenced_schedule_was_not_found'));
    }

    function get_live_schedule_user($id){
        if($id) return $this->db->get_where('live_schedule_user',array('id'=>$id))->row_array();
    }

    function get_all_live_schedule_user($user_id,$live_schedule_id){
        if($user_id) $payl['user_id']=$user_id;
        if($live_schedule_id) $payl['live_schedule_id']=$live_schedule_id;
        if($payl) return $this->db->get_where('live_schedule_user',array('user_id'=>$user_id,'live_schedule_id'=>$live_schedule_id));
    }

    function delete_live_schedule_user_api($id){
        $resp=['status'=>'fail'];
        if($id) {
            //$live_schedule_user=$this->get_live_schedule_user($id);

            $this->db->where('id', $id);
            $this->db->delete('live_schedule_user');

            $resp['status']='success';

            //if($this->db->affected_rows());// $this->email_model->delete_live_schedule_user_notification($live_schedule_course_id,$live_schedule_start_time,$live_schedule_duration,$live_schedule_platform);

            //$this->session->set_flashdata('flash_message', get_phrase('user_has_been_deleted'));
        }
        //else $this->session->set_flashdata('error_message', get_phrase('referenced_user_was_not_found'));
        return $resp;
    }

    function add_live_schedule_user($live_schedule_id){
        $user_ids=$this->input->post('user_id');
        $_success=0;
        $_users_length=is_array($user_ids)?count($user_ids):0;
        $live_schedule=$this->get_live_schedule($live_schedule_id);
        if($live_schedule && $_users_length){
            foreach($user_ids as $user_id){
                $user=$this->user_model->get_all_user($user_id)->row_array();
                if($user){
                    $data=array('user_id'=>$user_id,'live_schedule_id'=>$live_schedule_id);
                    @$this->db->insert('live_schedule_user',$data);

                    $live_schedule_user_id = $this->db->insert_id();
                    if($live_schedule_user_id){
                        $_success++;
                        if($live_schedule_user_id) {
                            
                            $this->email_model->send_email($user['email'],"You were added to an event","You have been added to {$live_schedule['title']} scheduled event starting at {$live_schedule['start_time']} GMT on {$live_schedule['platform']} platform");
                            //$this->email_model->create_live_schedule_notification_for_user_add($live_schedule_id,$user_id);
                        }
                    }
                }
            }

            $this->session->set_flashdata('flash_message', get_phrase($_success.'/'.$_users_length.' _users_has_been_added_to_the_event'));
        }
        else {
            $msg=$live_schedule?"select_users_to_add":"schedule_not_available";
            $this->session->set_flashdata('error_message', get_phrase($msg));
        }
    }

    function fetch_assignments($course_ids = "",$deadline_date=0,$deadline_time=0,$_comp=0){
        $resp=array();
        $course_ids=(array)$course_ids;
        foreach($course_ids as $course_id){
            if($course_id) $this->db->where('course_id', $course_id);

            $comp=$_comp>0?'>':($_comp<0?'<':null);
            if($deadline_time) $this->db->where("deadline_time$comp", $deadline_time);
            if($deadline_date) $this->db->where("deadline_date$comp", $deadline_date);
            $this->db->where('status','active');

            $this->db->order_by('assignment_id', 'desc');
            $resp=array_merge($resp,$this->db->get('assignment')->result_array());
        }
        return $resp;
    }

    function get_assignment_submision($user_id=0,$assignment_id=0){
        return $this->db->get_where('assignment_submission', array('user_id' => $user_id, 'assignment_id' => $assignment_id))->row_array();
    }


    function add_activity_log($user_id,$priority,$controller,$function,$option,$action,$parameter,$model,$message,$description=null,$feedback=null,$note=null){
        if($controller && $action){
            $data=array('user_id'=>$user_id,'priority'=>$priority,'controller'=>$controller,'function'=>$function,'option'=>$option,'action'=>$action,'parameter'=>$parameter,'model'=>$model,'message'=>$message,'description'=>$description,'feedback'=>$feedback,'note'=>json_encode($note));
            $this->db->insert('activity_log',$data);
        }
    }

    function fetch_activity_log($arr){
        ...
        $created=$arr['created'];
        $updated=$arr['updated'];
        $priority_1=$arr['priority_1'];
        $priority_2=$arr['priority_2'];
        $created_1=$arr['created_1'];
        $created_2=$arr['created_2'];
        $updated_1=$arr['updated_1'];
        $updated_2=$arr['updated_2'];
        ...
        if($id) $data['id']=$id;
            ...
            if($function) $data['function']=$function;
            if($option) $data['option']=$option;
            if($action) $data['action']=$action;
            if($model) $data['model']=$model;
            if($created) $data['created']=$created;
            if($updated) $data['updated']=$updated;
            if(strlen($priority)) $data['priority']=$priority;

            if(strlen($priority_1)) $data['priority>']=$priority_1;
            if(strlen($priority_2)) $data['priority<']=$priority_2;
            if($created_1) $data['created>']=$created_1;
            if($created_2) $data['created<']=$created_2;
            if($updated_1) $data['updated>']=$updated_1;
            if($updated_2) $data['updated<']=$updated_2;

            if($parameter) $this->db->like('parameter',$parameter);
            if($message) $this->db->like('message',$message);
            if($description) $this->db->like('description',$description);
            if($feedback) $this->db->like('feedback',$feedback);
            if($note) $this->db->like('note',$note);

            if($order_by) $this->db->order_by($order_by,$_pol?'ASC':'DESC');
        }
        return $this->db->get_where('activity_log',$data)->result_array();
    }

    function add_key_activity_log(){
        $user_id=$this->session->userdata('user_id');
        ...
        $note=$this->input->post('note');
        if($user_id && $activity){
            $data=array('user_id'=>$user_id,'to_user'=>$to_user,'priority'=>$priority,'activity'=>$activity,'description'=>$description,'note'=>$note);
            if($this->db->insert('key_activity_log',$data)){
                $this->session->set_flashdata('flash_message', get_phrase('new_key_activity_log_has_been_created'));
            }
            else $this->session->set_flashdata('error_message', get_phrase('system_error'));
        }
        else $this->session->set_flashdata('error_message', get_phrase('enter_all_neccesary_field'));
    }

    function fetch_key_activity_log($arr){
        ...
        if($id) $data['id']=$id;
            ...
            if(strlen($to_user)) $data['to_user']=$to_user;
            if(strlen($priority)) $data['priority']=$priority;
            ...
            if($activity) $this->db->like('activity',$activity);
            if($description) $this->db->like('description',$description);
            if($note) $this->db->like('note',$note);

            if($order_by) $this->db->order_by($order_by,$_pol?'ASC':'DESC');
        }
        return $this->db->get_where('key_activity_log',$data)->result_array();
    }

    function update_key_activity_log($id){
        //$to_user=(int)$this->input->post('to_user');
        $activity=htmlspecialchars_($this->input->post('activity'));
        $description=htmlspecialchars_($this->input->post('description'));
        $note=$this->input->post('note');
        if($id){
            //$key_activity_log=$this->fetch_key_activity_log(array('id'=>$id));
            $data=array('activity'=>$activity,'description'=>$description,'note'=>$note);
            $this->db->where('id', $id);
            $this->db->update('key_activity_log',$data);
            ...
            $this->session->set_flashdata('flash_message', get_phrase('key_activity_log_has_been_updated'));
        }
        else $this->session->set_flashdata('error_message', get_phrase('referenced_key_activity_log_was_not_found'));
    }

    function delete_key_activity_log($id){
        $resp=['status'=>'fail'];
        if($id) {
            $this->db->where('id', $id);
            $this->db->delete('key_activity_log');
    ...
    function add_report_qa(){
        ...
        if($user_id && $title){
            ...
            if (isset($_FILES['file']) && $_FILES['file']['name'] != "") {
                ...
                $accepted_ext = array('pdf', 'zip','png');
                $path = $_FILES['file']['name'];
                $size = $_FILES['file']['size'];
                $ext = strtolower(pathinfo($path, PATHINFO_EXTENSION));
                if (in_array($ext, $accepted_ext)) {
    
                    $doc_name = md5(rand(1000, 100000)) . '.'.$ext;
    
                    if(move_uploaded_file($_FILES['file']['tmp_name'], 'uploads/system/' . $doc_name)) $file='uploads/system/' . $doc_name;
                }
            }

            $data=array('user_id'=>$user_id,'to_user'=>$to_user,'code'=>$code,'title'=>$title,'file'=>$file,'description'=>$description,'note'=>$note);
            if($this->db->insert('report_qa',$data)){
                $this->session->set_flashdata('flash_message', get_phrase('new_report_qa_has_been_created'));
            }
            else $this->session->set_flashdata('error_message', get_phrase('system_error'));
        }
        else $this->session->set_flashdata('error_message', get_phrase('enter_all_neccesary_field'));
    ...
    function fetch_report_qa($arr){
        ...
        if($id) $data['id']=$id;
            ...
            if(strlen($to_user)) $data['to_user']=$to_user;
            if(strlen($code)) $data['code']=$code;

            if(strlen($code_1)) $data['code>=']=$code_1;
            if(strlen($code_2)) $data['code<=']=$code_2;
            if($created_1) $data['created>']=$created_1;
            if($created_2) $data['created<']=$created_2;
            if($updated_1) $data['updated>']=$updated_1;
            if($updated_2) $data['updated<']=$updated_2;
        ...
        return $this->db->get_where('report_qa',$data)->result_array();
    }

    function update_report_qa($id){
        $description=htmlspecialchars_($this->input->post('description'));
        $note=$this->input->post('note');
        $code=$this->input->post('code');
        $title=$this->input->post('title');
        if($id){
            $report_qa=$this->fetch_report_qa(['id'=>$id])[0];
            //var_dump($_FILES['file']);
            if (isset($_FILES['file']) && $_FILES['file']['name'] != "") {

                if (file_exists($report_qa['file'])) {
                    unlink($report_qa['file']);
                }
                $accepted_ext = array('pdf', 'zip','png','ppt','pptx');
                $path = $_FILES['file']['name'];
                $size = $_FILES['file']['size'];
                $ext = strtolower(pathinfo($path, PATHINFO_EXTENSION));
                if (in_array($ext, $accepted_ext)) {
            ...
            //$report_qa=$this->fetch_report_qa(array('id'=>$id));
            $data=array('description'=>$description,'title'=>$title,'note'=>$note,'code'=>$code);
            if($file) $data['file']=$file;
            $this->db->where('id', $id);
            $this->db->update('report_qa',$data);
            ...
            $this->session->set_flashdata('flash_message', get_phrase('report_qa_has_been_updated'));
        }
        else $this->session->set_flashdata('error_message', get_phrase('referenced_report_qa_was_not_found'));
    }

    function delete_report_qa($id){
        $resp=['status'=>'fail'];
        if($id) {
            $this->db->where('id', $id);
            $this->db->delete('report_qa');
    ...
    function add_review_qa(){
        ...
        $note=$this->input->post('note');
        if($user_id){
            $review_qa=$this->fetch_review_qa(array('user_id'=>$user_id,'to_user'=>$to_user))[0];
            //var_dump($review_qa);
            if($review_qa) return $this->update_review_qa($review_qa['id']);

            $data=array('user_id'=>$user_id,'to_user'=>$to_user,'code'=>$code,'description'=>$description,'note'=>$note);
            if($this->db->insert('review_qa',$data)){
                $this->session->set_flashdata('flash_message', get_phrase('new_review_qa_has_been_created'));
            }
            else $this->session->set_flashdata('error_message', get_phrase('system_error'));
        }
        else $this->session->set_flashdata('error_message', get_phrase('enter_all_neccesary_field'));
    ...
    function fetch_review_qa($arr){
        ...
        $this->db->select('*, AVG(code) as average_rate');
        $result=$this->db->get_where('review_qa',$data)->result_array();
        if(!$result[0]['id']) array_shift($result);
        return $result;
    }

    function update_review_qa($id){
        $description=htmlspecialchars_($this->input->post('description'));
        $note=$this->input->post('note');
        $code=$this->input->post('code');
        if($id){
            //$review_qa=$this->fetch_review_qa(array('id'=>$id));
            $data=array('description'=>$description,'note'=>$note,'code'=>$code);
            $this->db->where('id', $id);
            $this->db->update('review_qa',$data);
            ...
            $this->session->set_flashdata('flash_message', get_phrase('review_qa_has_been_updated'));
        }
        else $this->session->set_flashdata('error_message', get_phrase('referenced_review_qa_was_not_found'));
    }

    function delete_review_qa($id){
        $resp=['status'=>'fail'];
        if($id) {
            $this->db->where('id', $id);
            $this->db->delete('review_qa');
    ...
    function update_qa_courses()
    {
        $resp=['success'=>'updated'];
        $qa_courses_array = (array)$this->input->post('qa_courses_array');
        $qa_courses_map=array_fill_keys($qa_courses_array,true);
        $qa_courses_map_json=json_encode($qa_courses_map);
        
        $row = $this->db->where('key', 'qa_courses_map_json')->get('settings');
        if ($row->num_rows() > 0) {
            $this->db->where('key', 'qa_courses_map_json')->update('settings', ['value' => $qa_courses_map_json]);
        } else {
            $this->db->insert('settings', ['key' => 'qa_courses_map_json', 'value' => $qa_courses_map_json]);
        }
        return $resp;
    }

    function get_rating_by_status($ratable_type='course',$ratable_id=null,$status=null){
        $payl=array('ratable_type' => $ratable_type);
        if(strlen($status)) $payl['status']=$status;
        if(strlen($ratable_id)) $payl['ratable_id']=$ratable_id;
        return $this->db->order_by('id', 'desc')->get_where('rating',$payl );
    }

    function update_rating_status($id,$status)
    {
        $resp=['status'=>'success', 'success'=>'updated'];
        $this->db->where('id', $id)->update('rating', ['status' => $status]);
        return $resp;
    }
